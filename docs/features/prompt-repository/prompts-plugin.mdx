...
<Note>
On enterprise, a matching [prompt deployment](/enterprise/prompt-deployments) selects the version server-side and overrides `x-bf-prompt-version`. The header is honoured only when no deployment matches.
</Note>
...
- [Prompt Deployments](/enterprise/prompt-deployments) - enterprise server-side version selection with CEL rules and traffic splits.
