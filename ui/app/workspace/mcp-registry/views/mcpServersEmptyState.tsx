	...
	return (
		<div className="flex min-h-[80vh] w-full flex-col items-center justify-center gap-4 py-16 text-center">
			...
			<div className="flex flex-col gap-1">
				<h1 className="text-muted-foreground text-xl font-medium">{t("MCP servers connect tools and context to the gateway")}</h1>
				...
				<div className="mx-auto mt-6 flex flex-row flex-wrap items-center justify-center gap-2">
					<Button
						variant="outline"
						aria-label={t("Read more about MCP servers (opens in new tab)")}
						data-testid="mcp-registry-button-read-more"
						onClick={() => {
							window.open(`${MCP_SERVERS_DOCS_URL}?utm_source=bfd`, "_blank", "noopener,noreferrer");
						}}
					>
						{t("Read more")} <ArrowUpRight className="text-muted-foreground h-3 w-3" />
					</Button>
					<Button
						aria-label={t("Add your first MCP server")}
						onClick={onAddClick}
						disabled={!canCreate}
					...
					<Button asChild aria-label={t("Browse the MCP server library")} data-testid="mcp-library-empty-link-btn">
