...
export const OAuth2Authorizer: React.FC<OAuth2AuthorizerProps> = ({
	...
	const checkOAuthStatus = useCallback(async () => {
		...
		try {
			...
			console.error(t("Error checking OAuth status:"), error);
		}
				...
				<div className="space-y-3 px-5 py-4">
					...
					{status === "confirm" && (
						<>
							<InfoBox icon={<KeyRound className="size-4" />}>
								<p>
									{t("We'll open {host} to {action} the OAuth setup{discover}.", {
										host: authorizationHost,
										action: isPerUserReauth ? t("renew") : t("verify"),
										discover: isPerUserReauth ? "" : t(" and discover available tools"),
									})}
								</p>
					...
					{status === "polling" && (
						<>
							<InfoBox icon={<Loader2 className="size-4 animate-spin" />}>
								<p>{t("This dialog will update automatically once the provider redirects back.")}</p>
								<p className="text-muted-foreground/80 text-xs">{t("Keep the popup open until authorization is complete.")}</p>
							</InfoBox>
					...
					{status === "blocked" && (
						<>
							<InfoBox variant="warning" icon={<AlertTriangle className="size-4" />}>
								<p>{t("Your browser prevented the authorization window from opening.")}</p>
								<p className="text-xs opacity-80">{t("Enable popups for this site in your browser settings, then try again.")}</p>
							</InfoBox>
							<div className="flex justify-end gap-2">
					...
					{status === "success" && (
						<InfoBox variant="success" icon={<CheckCircle2 className="size-4" />}>
							<p className="font-medium">{t("Finishing setup and syncing available tools.")}</p>
							<p className="text-xs opacity-80">{t("You can close this dialog; setup will complete in the background.")}</p>
						</InfoBox>
					)}
					...
					{status === "failed" && (
						<>
							<InfoBox variant="danger" icon={<XCircle className="size-4" />}>
								<p className="font-medium">{t("Authorization did not complete.")}</p>
