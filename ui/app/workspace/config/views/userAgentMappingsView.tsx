...
export default function UserAgentMappingsView({ disabled }: UserAgentMappingsViewProps) {
	...
	const handleSubmit = async () => {
		...
		try {
			if (editingMappingId) {
				...
				toast.success(t("User agent mapping updated."));
			} else {
				await createMapping(validated).unwrap();
				toast.success(t("User agent mapping added."));
			}
			handleSheetOpenChange(false);
		} catch (error) {
			toast.error(`Failed to ${editingMappingId ? t("update") : t("add")} mapping: ${getErrorMessage(error)}`);
		}
	};
	...
	const handleDelete = async (id: string) => {
		try {
			...
			toast.success(t("User agent mapping deleted."));
		} catch (error) {
	...
	return (
		<div className="space-y-4">
			<div className="flex items-start justify-between gap-4">
				<div>
					<h3 className="text-lg font-semibold tracking-tight">{t("User Agent Mappings")}</h3>
					<p className="text-muted-foreground text-sm">
						{t("Map incoming User-Agent strings to app names and optional logos used in logs.")}
					</p>
				</div>
				<div className="pt-2">
					<Button
			...
			<Sheet open={isSheetOpen} onOpenChange={handleSheetOpenChange}>
				<SheetContent className="p-0">
					<SheetHeader className="flex flex-col items-start px-4 pt-6 md:px-6">
						<SheetTitle>{isEditing ? t("Edit User Agent Mapping") : t("Add User Agent Mapping")}</SheetTitle>
						<SheetDescription>{t("Define how a User-Agent value maps to an app label in logs.")}</SheetDescription>
					</SheetHeader>
					...
					<SheetFooter className="flex-row justify-end border-t px-4 py-4 md:px-6">
						...
						<Button type="button" onClick={handleSubmit} disabled={controlsDisabled} data-testid="user-agent-mapping-submit-btn">
							{isEditing ? t("Save Changes") : t("Add Mapping")}
						</Button>
					</SheetFooter>
				</SheetContent>
			</Sheet>
			...
			<Table containerClassName="rounded-sm border">
				...
				<TableBody>
					{isLoading ? (
						...
						<TableRow>
							<TableCell colSpan={6} className="text-muted-foreground py-6 text-center">
								{t("No user agent mappings configured.")}
							</TableCell>
						</TableRow>
					) : (
								...
								<TableRow key={mapping.id}>
									...
									<TableCell>
										<span className={mapping.is_active ? "text-sm text-emerald-700" : "text-muted-foreground text-sm"}>
											{mapping.is_active ? t("Active") : t("Inactive")}
										</span>
									</TableCell>
									<TableCell className="text-right">
										<AlertDialog>
											...
											<AlertDialogContent>
												<AlertDialogHeader>
													<AlertDialogTitle>{t("Are you sure you want to delete this mapping?")}</AlertDialogTitle>
													<AlertDialogDescription>
														{t("This action cannot be undone. This will permanently delete the user agent mapping.")}
													</AlertDialogDescription>
												</AlertDialogHeader>
												<AlertDialogFooter>
		...
		<div className="space-y-4">
			<div className="space-y-2">
				...
				<Input
					...
					placeholder={t("User-Agent string or regex")}
					value={draft.pattern}
			...
			<div className="flex items-center justify-between rounded-sm border p-3">
				<div>
					...
					<p className="text-muted-foreground text-xs">{t("Inactive mappings are saved but ignored by detection.")}</p>
				</div>
				<Switch
	...
	return (
		<div className="flex items-center gap-2">
			...
			<Button type="button" variant="outline" size="icon" disabled={disabled} asChild>
				<label aria-label={t("Upload logo")}>
					...
					<input
						...
						onChange={async (event) => {
							...
							if (file.size > MAX_LOGO_BYTES) {
								toast.error(t("Logo must be 256KB or smaller."));
								event.target.value = "";
								return;
							}
							try {
								...
								toast.error(t("Failed to read logo file."));
							}
							event.target.value = "";
						}}
					/>
