...
export default function LoggingView() {
	...
	const handleSave = useCallback(async () => {
		if (!bifrostConfig) {
			toast.error(t("Configuration not loaded"));
			return;
		}

		// Validate log retention days
		if (localConfig.log_retention_days < 1) {
			toast.error(t("Log retention days must be at least 1 day"));
			return;
		}

		try {
			await updateCoreConfig({ ...bifrostConfig, client_config: localConfig }).unwrap();
			toast.success(t("Logging configuration updated successfully."));
		} catch (error) {
	...
	return (
		<div className="mx-auto w-full max-w-4xl space-y-4 px-4 py-6 md:px-0">
			<PageTitle title={t("Logs Settings")}>{t("Configure logging settings for requests and responses.")}</PageTitle>
			...
			<div className="space-y-4">
					...
					<div className="flex items-center justify-between space-x-2 rounded-sm border p-4">
						<div className="space-y-0.5">
							...
							<p className="text-muted-foreground text-sm">
								{t("Enable logging of requests and responses to a SQL database. This can add 40-60mb of overhead to the system memory.")}
								{!bifrostConfig?.is_logs_connected && (
									<span className="text-destructive font-medium">
										{" "}
										{t("Requires logs store to be configured and enabled in config.json.")}
									</span>
								)}
							</p>
						</div>
				...
				<div className="min-w-0 space-y-3 rounded-sm border p-4" data-testid="workspace-hidden-request-types">
					<div className="space-y-0.5">
						...
						<p className="text-muted-foreground text-sm">
							...
							<code className="text-xs">{"client.hidden_request_types"}</code> {t("in config.json.")}
						</p>
					</div>
					{isConfigError ? (
						<p className="text-destructive text-sm" role="alert">
							{t("Unable to load hidden request types.")}
						</p>
					) : (
						<MultiSelect
							...
							placeholder={isConfigLoading ? t("Loading configuration…") : t("All request types are visible")}
						...
						<div className="flex items-center justify-between space-x-2 rounded-sm border p-4">
							<div className="space-y-0.5">
								...
								<p className="text-muted-foreground text-sm">
									...
									<code className="text-xs">{"store_raw_request_response"}</code> {t("is on. Raw-byte send-back to callers via")}{" "}
									<code className="text-xs">{"send_back_raw_*"}</code> {t("is unaffected.")}
								</p>
							</div>
							<Switch
					...
					<div className="flex items-center justify-between space-x-2 rounded-sm border p-4">
						<div className="space-y-0.5">
							<label htmlFor="retain-content-in-object-storage" className="text-sm font-medium">
								{t("Retain Content in Object Storage")}
							</label>
							<p className="text-muted-foreground text-sm">
								{t("When enabled, requests with content logging disabled (via the global setting above or the")}{" "}
								...
								{!bifrostConfig?.is_object_storage_connected && (
									<span className="text-destructive font-medium">
										{" "}
										{t("Requires object storage to be configured on the logs store in config.json.")}
									</span>
								)}
							</p>
						</div>
					...
					<div className="flex items-center justify-between space-x-2 rounded-sm border p-4">
						<div className="space-y-0.5">
							<label htmlFor="allow-per-request-content-storage-override" className="text-sm font-medium">
								{t("Allow Per-Request Content Storage Override")}
							</label>
							<p className="text-muted-foreground text-sm">
								{t("When enabled, individual requests can override the global content logging setting using the")}{" "}
								<code className="text-xs">{"x-bf-disable-content-logging"}</code>{" "}
								{t("header or context key, and can opt-in to persisting raw provider bytes in logs using the")}{" "}
								<code className="text-xs">{"x-bf-store-raw-request-response"}</code>{" "}
								{t("header. Raw-byte storage requires content logging to be on, either globally, or via")}{" "}
								<code className="text-xs">{"x-bf-disable-content-logging: false"}</code>{" "}
								{t("on the same request. If content logging is off, raw bytes are dropped from the log record even when")}{" "}
								<code className="text-xs">{"x-bf-store-raw-request-response: true"}</code>
								{t(". Does not control sending raw bytes back to callers; see Allow Per-Request Raw Override.")}
							</p>
						</div>
						<Switch
				...
				<div className="flex items-center justify-between space-x-2 rounded-sm border p-4">
					<div className="space-y-0.5">
						<label htmlFor="allow-per-request-raw-override" className="text-sm font-medium">
							{t("Allow Per-Request Raw Override")}
						</label>
						<p className="text-muted-foreground text-sm">
							{t("When enabled, individual requests can send raw provider request/response bytes back to the caller using the")}{" "}
							<code className="text-xs">{"x-bf-send-back-raw-request"}</code> {t("and")}{" "}
							<code className="text-xs">{"x-bf-send-back-raw-response"}</code>{" "}
							{t(
				...
				{localConfig.enable_logging && bifrostConfig?.is_logs_connected && (
					<div className="flex items-center justify-between space-x-2 rounded-sm border p-4">
						<div className="space-y-0.5">
							...
							<p className="text-muted-foreground text-sm">
								{t("Number of days to retain logs in the database. Minimum is 1 day. Older logs will be automatically deleted.")}
							</p>
						</div>
						<Input
				...
				<div className="flex items-center justify-between space-x-2 rounded-sm border p-4">
					<div className="space-y-0.5">
						<label htmlFor="hide-deleted-virtual-keys-in-filters" className="text-sm font-medium">
							{t("Do Not Show Deleted VirtualKeys In Filters")}
						</label>
						<p className="text-muted-foreground text-sm">
							{t("When enabled, deleted virtual keys are excluded from Virtual Keys filter options in Logs, Dashboard, and MCP Logs.")}
						</p>
					</div>
					<Switch
				...
				{localConfig.enable_logging && bifrostConfig?.is_logs_connected && (
					<div className="space-y-2 rounded-sm border p-4">
						...
						<p className="text-muted-foreground text-sm">
							{t("Comma-separated list of request headers to capture in log metadata. Supports exact names and wildcard patterns (e.g.")}{" "}
							<code className="text-xs">{"x-custom-*"}</code> {t("captures all headers with that prefix,")}{" "}
							<code className="text-xs">*</code> {t("logs all headers; note that")} <code className="text-xs">*</code>{" "}
							{t(
							...
							<code className="text-xs">{"x-bf-lh-"}</code> {t("prefix are always captured automatically.")}
						</p>
						<Textarea
							id="logging-headers"
							data-testid="workspace-logging-headers-textarea"
							className="h-24"
							placeholder={t("X-Tenant-ID, X-Request-Source, x-custom-*")}
...
const RestartWarning = () => {
	const { t } = useLocaleCtx();
	return <div className="text-muted-foreground mt-2 pl-4 text-xs font-semibold">{t("Need to restart Bifrost to apply changes.")}</div>;
};