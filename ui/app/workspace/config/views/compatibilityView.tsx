...
export default function CompatibilityView() {
	...
	const handleSave = useCallback(async () => {
		...
		try {
			...
			toast.success(t("Compatibility settings updated successfully."));
		} catch (error) {
	...
	return (
		<div className="mx-auto w-full max-w-4xl space-y-6">
			<PageTitle title={t("Compatibility")}>
				{t("Configure request conversions and compatibility fallbacks.")}{" "}
				<a
				...
				<div className="flex items-center justify-between space-x-2">
					<div className="space-y-0.5">
						<label htmlFor="compat-convert-text-to-chat" className="text-sm font-medium">
							{t("Convert Text to Chat")}
						</label>
						<p className="text-muted-foreground text-sm">
							{t("Convert text completion requests to chat for models that only support chat.")}
						</p>
					</div>
					<Switch
				...
				<div className="flex items-center justify-between space-x-2">
					<div className="space-y-0.5">
						<label htmlFor="compat-convert-chat-to-responses" className="text-sm font-medium">
							{t("Convert Chat to Responses")}
						</label>
						<p className="text-muted-foreground text-sm">
							{t("Convert chat completion requests to responses for models that only support responses.")}
						</p>
					</div>
					<Switch
				...
				<div className="flex items-center justify-between space-x-2">
					<div className="space-y-0.5">
						<label htmlFor="compat-should-drop-params" className="text-sm font-medium">
							{t("Drop Unsupported Params")}
						</label>
						<p className="text-muted-foreground text-sm">{t("Drop unsupported parameters based on model catalog allowlist.")}</p>
					</div>
					<Switch
				...
				<div className="flex items-center justify-between space-x-2">
					<div className="space-y-0.5">
						<label htmlFor="compat-should-convert-params" className="text-sm font-medium">
							{t("Convert Unsupported Param Values")}
						</label>
						<p className="text-muted-foreground text-sm">{t("Converts model parameter values that are not supported by the model.")}</p>
					</div>
					<Switch
				...
				<div className="flex items-center justify-between space-x-2">
					<div className="space-y-0.5">
						<label htmlFor="compat-azure-deepseek" className="text-sm font-medium">
							{t("Use Chat Completion APIs for Azure Deepseek models")}
						</label>
						<p className="text-muted-foreground text-sm">
							{t("Use Chat Completion APIs for Claude Code, Codex, etc. for Azure Deepseek models.")}
						</p>
					</div>
					<Switch
