	...
	if (isCreate && !stagedCreate) {
		return (
			<div className="text-muted-foreground rounded-md border border-dashed p-6 text-center text-sm">
				{t("Create this Virtual MCP first, then assign it to virtual keys here.")}
			</div>
		);
	}
		...
		<div className="flex flex-col gap-4">
			<div className="flex items-center justify-between">
				<div className="flex flex-col gap-0.5">
					<Label>{t("Virtual keys")}</Label>
					<p className="text-muted-foreground text-xs">{t("Keys assigned here can reach this Virtual MCP at its endpoint.")}</p>
				</div>
				<VirtualKeySelector
					mode="add"
			...
			{isLoading ? (
				<div className="text-muted-foreground rounded-md border border-dashed p-6 text-center text-sm">{t("Loading virtual keys…")}</div>
			) : value.length === 0 ? (
				<div className="text-muted-foreground rounded-md border border-dashed p-6 text-center text-sm">
					{t("No virtual keys assigned yet. Assign one so callers can reach this Virtual MCP.")}
				</div>
			) : (
