...
export default function VirtualMCPsTable({
	...
	const confirmDelete = async () => {
		...
		try {
			...
			toast({ title: t("Virtual MCP deleted") });
		} catch (err) {
			toast({ title: t("Failed to delete Virtual MCP"), description: getErrorMessage(err), variant: "destructive" });
		} finally {
			setPendingId(null);
		}
	};
	...
	return (
		<div className="flex grow flex-col overflow-auto">
			<AlertDialog open={pendingDelete !== null} onOpenChange={(open) => !open && setPendingDelete(null)}>
				<AlertDialogContent>
					<AlertDialogHeader>
						<AlertDialogTitle>{t("Delete this Virtual MCP?")}</AlertDialogTitle>
						<AlertDialogDescription>
							{t(
								"{name} will stop being served at {endpoint} and will be removed from any virtual keys it is assigned to. This cannot be undone.",
								{
									name: pendingDelete?.name ?? "",
									endpoint: `/mcp/${pendingDelete?.endpoint_slug ?? ""}`,
								},
							)}
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
						<AlertDialogCancel data-testid="virtual-mcp-delete-cancel">{t("Cancel")}</AlertDialogCancel>
			...
			<PageTitle title={t("Virtual MCPs")}>
				{t("Bundle tools from your MCP servers into a single endpoint, then assign it to virtual keys.")}
			</PageTitle>
			...
			<div className="mb-4 flex items-center justify-between gap-3">
				<div className="relative max-w-sm min-w-[200px] flex-1">
					...
					<Input
						aria-label={t("Search Virtual MCPs")}
						placeholder={t("Search by name or endpoint...")}
						value={search}
					...
					<Table>
						<TableHeader className="bg-muted sticky top-0 z-20">
							<TableRow>
								...
								<TableHead>{t("Source servers")}</TableHead>
								<TableHead>{t("Included tools")}</TableHead>
							...
							{virtualMcps.length === 0 ? (
								<TableRow>
									<TableCell colSpan={8} className="h-24 text-center">
										{isFetching ? (
											<div className="text-muted-foreground text-sm">{t("Loading Virtual MCPs…")}</div>
										) : hasActiveSearch ? (
											<div className="text-muted-foreground text-sm">{t("No Virtual MCPs match your search.")}</div>
										) : (
											<div className="flex flex-col items-center gap-3 py-6">
												<Server className="text-muted-foreground h-10 w-10" strokeWidth={1} />
												<span className="text-muted-foreground text-sm">
													{t("No Virtual MCPs yet. Create one to bundle tools from your MCP servers into a single endpoint.")}
												</span>
	...
	return (
		<DropdownMenu>
			<DropdownMenuTrigger asChild>
				<Button variant="ghost" size="icon" className="h-8 w-8" aria-label={t("Virtual MCP actions")} disabled={busy}>
